...
def get_working_model():
    try:
        ...
        # Ищем 1.5-flash, если нет - берем первую доступную
...
if st.button("🚀 Сдать тест и получить ИИ-анализ"):
    # Проверка: ответил ли ученик на все вопросы
    if None in [q1, q2, q3, q4]:
        ...
        with st.spinner("ИИ анализирует ваши ответы..."):
            ...
            try:
                # Промпт для педагогического анализа
                analysis_prompt = f"""
                ...
                # Сохраняем в базу данных для учителя
...
# --- 5. КАБИНЕТ УЧИТЕЛЯ (АНАЛИТИКА) ---
st.markdown("---")
with st.expander("🔐 Вход для Учителя (Анализ данных)"):
    pass_input = st.text_input("Введите пароль доступа:", type="password")
    if pass_input == "admin":
        st.subheader("📊 Журнал ИИ-анализа результатов")
        if os.path.exists(DATA_FILE):
            df = pd.read_csv(DATA_FILE)
            st.dataframe(df)
            
            # Кнопка скачивания лога для исследования
            csv = df.to_csv(index=False).encode('utf-8')
            st.download_button("📥 Скачать таблицу анализа (CSV)", csv, "research_results.csv", "text/csv")
        else:
            st.info("Данных для анализа пока нет. Тест еще никто не проходил.")
